...
select * from dataset1 where state in ('Delhi' ,'Uttar Pradesh');
...
select * from (
select  * from topstates order by topstates.topstate desc limit 3) as a

union

select * from (
select * from bottomstates order by bottomstates.bottomstate asc limit 3) as b;
